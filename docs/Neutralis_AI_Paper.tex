...
The sycophancy problem and the broader question of AI ethics have both received significant attention in recent years. We draw on four areas.
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{img/thumbnail.png}}
...
In practice, this means the system develops a sense of what matters for a particular user or workspace. A team that frequently discusses hiring will find the system paying closer attention to discrimination patterns over time, without anyone needing to configure anything.

\section{Algorithm}

Algorithm~\ref{alg:neutralis} describes the full pipeline, from server startup through tool dispatch, weighted analysis, and pattern learning.

\begin{algorithm}[htbp]
\caption{Neutralis-AI: MCP-Based Ethical Oversight for AI Conversations}
\label{alg:neutralis}
\begin{algorithmic}[1]
\renewcommand{\algorithmicrequire}{\textbf{Input:}}
\renewcommand{\algorithmicensure}{\textbf{Output:}}
\REQUIRE $K = \{k_1, k_2, \ldots, k_n\}$ \COMMENT{Gemini API keys}
\REQUIRE $conv$ \COMMENT{conversation context from AI assistant}
\REQUIRE $req$ \COMMENT{original user request}
\REQUIRE $tool$ \COMMENT{tool name invoked via MCP}
\ENSURE $result$ \COMMENT{ethical analysis, bias report, guidance, or log confirmation}

\STATE \textbf{// Server Initialization}
\STATE $keys \leftarrow \text{parse}(K)$; $idx \leftarrow 0$
\STATE $gemini \leftarrow \text{initModel}(keys[idx])$
\STATE $memory \leftarrow \text{loadConcerns}()$ \COMMENT{from file or in-memory}
\STATE Register tools: \{ethics\_check, critical\_thinking, ethics\_guide, ethics\_learn\}
\STATE Start MCP transport over stdio

\STATE
\STATE \textbf{// On Tool Invocation}
\STATE \textbf{function} \textsc{HandleRequest}($tool, conv, req, ctx$)
\IF{$tool$ = ``ethics\_check''}
    \STATE $W \leftarrow \text{\textsc{GetWeighted}}(memory, ctx, \{0.25, 0.35, 0.20, 0.20\})$
    \STATE $prompt \leftarrow \text{buildEthicsPrompt}(conv, req, W)$
    ...
    \IF{autoStore = true}
        ...
        \ENDFOR
    \ENDIF
\ELSIF{$tool$ = ``critical\_thinking''}
    \STATE $W_b \leftarrow \text{\textsc{GetWeighted}}(memory, ctx, \{0.40, 0.20, 0.30, 0.10\})$
    \STATE $W_s \leftarrow \text{\textsc{GetSessionPatterns}}(sessionId)$
    \STATE $prompt \leftarrow \text{buildBiasPrompt}(conv, req, W_b, W_s)$
    \STATE $result \leftarrow \text{\textsc{CallGemini}}(prompt)$
\ELSIF{$tool$ = ``ethics\_guide''}
    \STATE $W_g \leftarrow \text{\textsc{GetWeighted}}(memory, ctx, \{0.25, 0.25, 0.35, 0.15\})$
    \STATE $stats \leftarrow \text{getCategoryStats}(memory)$
    \STATE $prompt \leftarrow \text{buildGuidePrompt}(scenario, domain, W_g, stats)$
    \STATE $result \leftarrow \text{\textsc{CallGemini}}(prompt)$
\ELSIF{$tool$ = ``ethics\_learn''}
    \STATE \textsc{StoreConcern}($\{concern, category, severity, rec\}, memory$)
    \STATE $result \leftarrow \text{getCategoryStats}(memory)$
\ENDIF
\RETURN $result$

\STATE
\STATE \textbf{// Weighted Pattern Scoring}
\STATE \textbf{function} \textsc{GetWeighted}($memory, ctx, w$)
\FOR{each concern $c$ in $memory$}
    \STATE $r \leftarrow e^{-age(c) \;/\; (maxAge \times 0.3)}$ \COMMENT{recency}
    \STATE $s \leftarrow \text{severityMap}(c.severity)$ \COMMENT{0.25--1.0}
    \STATE $u \leftarrow c.successScore$ \COMMENT{default 0.5}
    \STATE $v \leftarrow \text{relevance}(c, ctx)$ \COMMENT{category + session + text sim}
    \STATE $c.total \leftarrow w_r \cdot r + w_s \cdot s + w_u \cdot u + w_v \cdot v$
\ENDFOR
\STATE Sort by $total$ descending; return top-$k$

\STATE
\STATE \textbf{// Store with Duplicate Detection}
\STATE \textbf{function} \textsc{StoreConcern}($c, memory$)
\FOR{each existing $e$ in $memory$ (last 24h)}
    \IF{$\text{similarity}(c, e) > 0.9$ \OR ($\text{sameSession}$ \AND $\text{similarity} > 0.8$)}
        \RETURN skip \COMMENT{duplicate}
    \ENDIF
\ENDFOR
\STATE Append $c$ to $memory$; persist to disk

\STATE
\STATE \textbf{// Gemini Call with Key Rotation}
\STATE \textbf{function} \textsc{CallGemini}($prompt$)
\FOR{$i = 1$ to $|keys|$}
    \STATE $idx \leftarrow (idx + 1) \mod |keys|$
    \STATE $resp \leftarrow \text{gemini}[idx].\text{generate}(prompt)$
    \IF{success}
        \RETURN $\text{parseJSON}(resp)$
    \ENDIF
\ENDFOR
\STATE \textbf{raise} ``All API keys exhausted''
\end{algorithmic}
\end{algorithm}
...
Privacy was the easiest category. Names, addresses, medical details---these have clear textual signals and Gemini caught most of them. Confirmation bias detection also worked well, because the structure of a sycophantic exchange (user states opinion, assistant agrees) is recognizable.
